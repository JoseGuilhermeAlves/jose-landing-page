...
/// Badge animado da tela de confirmacao Vitral — circulo preenchido
/// com check em duas linhas crescendo de 30% a 100% da animacao.
/// Painter recebe controller direto via `super(repaint:)` pra pular
/// build e layout no pipeline (ver CLAUDE.md).
class VitralConfirmationBadge extends StatefulWidget {
  const VitralConfirmationBadge({
  ...
  @override
  State<VitralConfirmationBadge> createState() =>
      _VitralConfirmationBadgeState();
}

class _VitralConfirmationBadgeState extends State<VitralConfirmationBadge>
  ...
  Widget build(BuildContext context) {
    return RepaintBoundary(
      child: SizedBox(
        ...
        height: widget.size,
        child: CustomPaint(
          painter: _VitralConfirmationBadgePainter(
...
class _VitralConfirmationBadgePainter extends CustomPainter {
  _VitralConfirmationBadgePainter({
  ...
  @override
  bool shouldRepaint(_VitralConfirmationBadgePainter old) {
